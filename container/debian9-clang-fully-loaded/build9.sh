...
$ ./build9.sh -p my-gcp-project -c debian9-clang-fully-loaded -t latest
will produce docker images in Google Container Registry:
    gcr.io/my-gcp-project/debian9-clang-fully-loaded:{latest, clang_revision}

To build locally:
$ ./build9.sh -l
will produce docker locally as debian9-clang-fully-loaded:latest
EOF
)
  echo "$usage"
}
...
main () {
  ...
  DIR="container/debian9-clang-fully-loaded"
  ...
  if [[ "$LOCAL" = true ]]; then
    ...
    bazel run //container/debian9-clang-fully-loaded:fl-toolchain
    echo "Testing container locally."
    bazel test //container/debian9-clang-fully-loaded:fl-toolchain-test
    echo "Tagging container."
    docker tag bazel/container/debian9-clang-fully-loaded:fl-toolchain debian9-clang-fully-loaded:latest
    echo -e "\n" \
      "debian9-clang-fully-loaded:lastest container is now available to use.\n" \
      "To try it: docker run -it debian9-clang-fully-loaded:latest \n"
  else
    ...
    gcloud container builds submit . \
      --config=${PROJECT_ROOT}/container/debian9-clang-fully-loaded/cloudbuild.yaml \
